    ...
    {{--@if (count($statusCheck) <= 0 && $currentUser->hasRole('adv_editor'))--}}
        ...
        <div class="form-check">
            <label for="status" class="col-lg-2 col-md-2 col-sm-2 form-check-label">
                <input type="radio" name="status" class="col-lg-2 col-md-2 col-sm-2 form-check-input" value="1" /> {{ $radioLabel1 }}
            </label>
            <label for="status" class="col-lg-2 col-md-2 col-sm-2 form-check-label">
                <input type="radio" name="status" class="col-lg-2 col-md-2 col-sm-2 form-check-input" value="0" /> {{ $radioLabel2 }}
            </label>
        </div>
        ...
        <div class="{{ $statusCheck[0]->checked == $advsetting->status ? 'bs-callout bs-callout-primary' : '' }}" >
            ...
            @if($statusCheck[0]->created_by == $currentUser->id && $displayForm)
                ...
                <div id="edit_status_check_form_{{ $statusCheck[0]->id }}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading_edit_status_check_form">
                    ...
                    <div class="form-check">
                            <label for="status" class="col-lg-2 col-md-2 col-sm-2 form-check-label">
                                <input type="radio" name="status" {{ $statusCheck[0]->checked == $currentStatusId  ? 'checked' : '' }} class="col-lg-2 col-md-2 col-sm-2 form-check-input" value="1" /> {{ $radioLabel1 }}
                            </label>
                            <label for="status" class="col-lg-2 col-md-2 col-sm-2 form-check-label">
                                <input type="radio" name="status" {{ $statusCheck[0]->checked == 1 ? 'checked' : '' }} class="col-lg-2 col-md-2 col-sm-2 form-check-input" value="0" /> {{ $radioLabel2 }}
                            </label>
                    </div>
            ...
            <div class="{{ $statusCk->checked == $currentStatusId ? 'bs-callout bs-callout-primary' : '' }}" >
                ...
                @if($statusCk->created_by == $currentUser->id && $displayForm)
                    ...
                    <div id="edit_status_check_form_{{ $statusCk->id }}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading_edit_status_check_form">
                        ...
                        <div class="form-check">
                                <label for="status" class="col-lg-2 col-md-2 col-sm-2 form-check-label">
                                    <input type="radio" name="status" class="col-lg-2 col-md-2 col-sm-2 form-check-input" {{ $statusCk->checked == $currentStatusId ? 'checked' : '' }} value="1" /> {{ $radioLabel1 }}
                                </label>
                                <label for="status" class="col-lg-2 col-md-2 col-sm-2 form-check-label">
                                    <input type="radio" name="status" class="col-lg-2 col-md-2 col-sm-2 form-check-input" {{ ($statusCk->checked == 1 || $statusCk->checked == 0) ? 'checked' : '' }} value="0" /> {{ $radioLabel2 }}
                                </label>
                        </div>
                        {!! Form::submit('保存', array('class'=>'btn btn-primary col-lg-offset-8 col-md-offset-8 col-sm-offset-8')) !!}
                        {!! Form::token() !!}
