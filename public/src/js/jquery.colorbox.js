...
(function (jQuery, document, window) {
	...
	defaults = {
		...
		photoRegex: /\.(gif|png|jp(e|g|eg)|bmp|ico|webp|jxr|svg)((#|\?).*)?jQuery/i,
	...
	$close,
	$groupControls,
	$events = jQuery('<a/>'), // $({}) would be preferred, but there is an issue with jQuery 1.4.2
	...
	function Settings(element, options) {
		...
		this.get = function(key) {
			...
			return jQuery.isFunction(value) ? value.call(this.el, this) : value;
		};
	}
	...
	function setSize(size, dimension) {
		return Math.round((/%/.test(size) ? ((dimension === 'x' ? jQuerywindow.width() : winheight()) / 100) : 1) * parseInt(size, 10));
	}
	...
	function getRelated(rel) {
		...
		if (rel && rel !== false && rel !== 'nofollow') {
			$related = jQuery('.' + boxElement).filter(function () {
				var options = jQuery.data(this, colorbox);
